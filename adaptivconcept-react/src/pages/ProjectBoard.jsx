              ...
              <div className="group bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 md:p-8 flex flex-col h-full transition-all hover:bg-white/[0.08] hover:border-white/20">
                <div className="mb-6 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="text-adaptiv-orange">
                      ...
                      {project.category === 'Automation' && <Terminal size={20} />}
                      {project.category === 'Fullstack' && <Cpu size={20} />}
                    </div>
                    <span className="text-xs uppercase font-bold tracking-[0.2em] text-gray-500">{project.category}</span>
                  </div>
                  {project.isHero && (
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {project.tags.map(tag => (
                    <span key={tag} className="px-3 py-1 rounded-lg bg-white/5 border border-white/5 text-[11px] text-gray-500 font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
