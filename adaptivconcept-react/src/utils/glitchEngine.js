...
void main () {
  ...
  // Rotate the coordinate frame used for the tear/block pattern so the
  // glitch reads at an angle while the sampled text itself stays upright.
  float ca = cos(uAngle);
  float sa = sin(uAngle);
  vec2 rot = vec2(
    (uv.x - 0.5) * ca - (uv.y - 0.5) * sa,
    (uv.x - 0.5) * sa + (uv.y - 0.5) * ca
  ) + 0.5;

  if (e > 0.001) {
    float band = floor(rot.y * uSlices);
    ...
    float sub = floor(rot.y * uSlices * 7.0);
  ...
  vec2 splitDir = vec2(ca, sa);
  float split = uRgbShift * e / uResolution.x;
  vec4 c = page(guv);
  float r = page(guv + splitDir * split).r;
  float b = page(guv - splitDir * split).b;
...
export function createGlitch(elements, options = {}) {
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        /* element capture is best-effort */
      }
    };
  }
  ...
  function render() {
    ...
    gl.uniform1f(uniforms.uAngle, (config.rotation || 0) * (Math.PI / 180));
  ...
  return {
    setOptions(next) {
      if (
        !Object.entries(next).some(([key, value]) => config[key] !== value)
      )
        return;
